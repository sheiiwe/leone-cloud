...
const supabaseBundle = fs.readFileSync(path.join(__dirname, '..', 'assets', 'supabase.js'), 'utf8')
const installerMac = fs.readFileSync(path.join(__dirname, '..', 'installa.sh'), 'utf8')
const updaterMac = fs.readFileSync(path.join(__dirname, '..', 'aggiorna.sh'), 'utf8')
...
assert.strictEqual(pkg.version, '1.16.1', 'versione riparazione Supabase non aggiornata')
assert.ok(pkg.build.files.includes('assets/**/*'), 'cartella assets esclusa dal pacchetto Electron')
assert.ok(supabaseBundle.length > 100_000 && supabaseBundle.includes('createClient'), 'bundle Supabase locale mancante o troncato')
assert.ok(ipc.match(/assets\/supabase\.js/g)?.length >= 2, 'bundle Supabase assente dagli aggiornamenti immediato e rapido')
assert.ok(html.includes('cdn.jsdelivr.net/gh/sheiiwe/leone-cloud@8dd2057da43a3a22f4e00d362c1054ceecf9da71/assets/supabase.js'), 'fallback Supabase immutabile mancante')
assert.ok(html.includes('const {createClient}=window.supabase'), 'inizializzazione Supabase non riferita esplicitamente a window')
assert.ok(installerMac.includes('Contents/Resources/app/assets/supabase.js'), 'installazione Mac senza verifica del bundle Supabase')
assert.ok(updaterMac.includes('Contents/Resources/app/assets/supabase.js'), 'aggiornamento Mac senza verifica del bundle Supabase')
